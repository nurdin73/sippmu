        ...
        <div class="row align-items-center">
            <div class="col-md-12">
                <div class="page-header-title">
                    <h5 class="m-b-10">Penerimaan Kas</h5>
                </div>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="<?php echo base_url();?>dashboard"><i class="feather icon-home"></i></a></li>
                    <li class="breadcrumb-item"><a href="#!">Transaksi Kasir</a></li>
                    <li class="breadcrumb-item"><a href="<?php echo base_url();?>transaction/penerimaan">Penerimaan Kas</a></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="col-sm-12">
        ...
        <div class="card">
            <div class="card-header">
                <form action="<?php echo site_url('transaction/penerimaan') ?>"  name="search_form" method="post" accept-charset="utf-8">
                    <div class="box-body">
                        ...
                        <div class="col-md-2 xright">
                            <?php if ($this->rbac->hasPrivilege('trx_penerimaan', 'can_add')) { ?>
            ...
            <div class="card-body">
                ...
                <div class="dt-responsive table-responsive">
                    <div class="download_label">Penerimaan Kas</div>
    ...
    <div class="modal-dialog modal-mid" role="document">
        <div class="modal-content modal-media-content">
            <div class="modal-header modal-media-header">
                <h4 class="box-title"> Tambah Penerimaan</h4> 
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>

            <div class="modal-body pt0 pb0">

                <form id="formadd" action="<?php echo site_url('transaction/penerimaan/add') ?>" name="form-area" method="post" accept-charset="utf-8"  enctype="multipart/form-data">
                    <div class="box-body">
...
<div class="modal fade" id="myModalEdit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-dialog2 modal-lg">
        <div class="modal-content modal-media-content">
            <div class="modal-header modal-media-header">
                <h4 class="box-title"> Detail Penerimaan</h4> 
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>

            <div class="modal-body pt0 pb0">

                <form id="formedit" action="<?php echo site_url('transaction/penerimaan/edit') ?>"  name="form-area" method="post" accept-charset="utf-8"  enctype="multipart/form-data">
                    <div class="box-body">
                    ...
                    <div class="box-footer">
                        <input type="hidden" id="penerimaan_id" name="penerimaan_id" value="" />
    ...
    <div class="modal-dialog modal-sm400">
        <div class="modal-content modal-media-content">
            ...
            <div class="modal-body pt0 pb0">
                ...
                <form id="form_detail" action="<?php echo site_url('transaction/penerimaan/do_detail') ?>" name="form-area" method="post" accept-charset="utf-8"  enctype="multipart/form-data">
                    <div class="box-body">
                    ...
                    <div class="box-footer">
                        ...
                        <input type="hidden" id="penerimaan_det_id" name="penerimaan_det_id" value="" />
                        <input type="hidden" id="penerimaan_id_detail" name="penerimaan_id" value="" />
...
$(document).ready(function (e) {
    ...
    $('#kode_trx_detail').on('select2:select', function (e) {
        ...
        $.ajax({
            dataType: 'json',
            url: '<?php echo base_url(); ?>transaction/penerimaan/get_kode_trx/' + value,
    ...
    var oTable = $("#dt_table").dataTable({
        ...
        serverSide: true,
        ajax: {
            "url": "<?php echo site_url('transaction/penerimaan/get_ajax') ?>", 
            "type": "POST",
            "data": function ( d ) {
                d.cabang = $('#cabang').val();
            ...
            if(total_det == 0){
                ...
                Swal.fire({
                  ...
                  if (result.value) {
                      $.ajax({
                            dataType: 'json',
                            url: '<?php echo base_url(); ?>transaction/penerimaan/closing_transaksi/' + id + '/' + cabang,
                            success: function (result) {
        ...
        if(cabang !== src_cabangx){
            ...
            Swal.fire({
              ...
              if (result.value) {
                  $.ajax({
                        dataType: 'json',
                        url: '<?php echo base_url(); ?>transaction/penerimaan/reopen_transaksi/' + id + '/' + cabang,
                        success: function (result) {
    ...
    var oTableDet = $("#dt_table_det").dataTable({
        ...
        serverSide: true,
        ajax: {
            "url": "<?php echo site_url('transaction/penerimaan/get_ajax_detail') ?>", 
            "type": "POST",
            "data": function ( d ) {
                d.cabang = $('#cabang').val();
                d.cabangx = $('#cabangx').val();
                d.penerimaan_id = $('#penerimaan_id').val();
            }
        },
        columns: [
            {"data": "nomor", width: "5%"},
...
function getEdit(event) {
    ...
    $.ajax({
        dataType: 'json',
        url: '<?php echo base_url(); ?>transaction/penerimaan/get_data/' + id,
        success: function (result) {
            
            $('#penerimaan_id').val($.trim(result.id));
...
function getAddDetail() {
    ...
    $('#act_detail').val('add');
    $('#penerimaan_det_id').val('');
    $('#cabang_detail').val($.trim(cabangxx));
    $('#penerimaan_id_detail').val($.trim(penerimaan_id));
...
function getEditDetail(event) {
    ...
    $.ajax({
        ...
        url: '<?php echo base_url(); ?>transaction/penerimaan/get_data_detail/' + id,

        success: function (result) {
            
            $('#act_detail').val('edit');
            $('#penerimaan_det_id').val($.trim(result.id));
            $('#cabang_detail').val($.trim(result.cabang));
            $('#penerimaan_id_detail').val($.trim(result.penerimaan));
            $('#tanggal_detail').val($.trim(tanggal_detail));
...
function deleteData(event) {
    ...
    if(cabang !== src_cabangx){
        ...
        if(status == 1){
            //delete_recordById('<?php echo base_url() ?>transaction/penerimaan/delete/' + id, '');
            Swal.fire({
              title: 'Peringatan!',
              ...
              if (result.value) {
                  $.ajax({
                        dataType: 'json',
                        url: '<?php echo base_url() ?>transaction/penerimaan/delete/' + id,
...
function deleteDataDetail(event) {
    ...
    if(cabang !== src_cabangx){
        ...
        if(status == 1){
            //delete_recordById('<?php echo base_url() ?>transaction/penerimaan/deleteDetail/' + id + '/' + penerimaan_id + '/' + akun_kas + '/' + cabang, '', 'no');
            var url = '<?php echo base_url() ?>transaction/penerimaan/deleteDetail/' + id + '/' + penerimaan_id + '/' + akun_kas + '/' + cabang;
            Swal.fire({
                  title: 'Peringatan!',
