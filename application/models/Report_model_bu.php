...
class Report_model extends CI_model {
    ...
    function getDataPenerimaanMuzakki($cabang, $src_date1, $src_date2, $src_muzakki='') {
        
        $tanggal1 = date('Y-m-d', strtotime($src_date1));
        $tanggal2 = date('Y-m-d', strtotime($src_date2));
        
        $this->db->select("a.id as idx, a.kode_transaksi, a.akun_kas, a.mutasi_kas_bank, a.jumlah, 
                        a2.no_transaksi, a2.tanggal, a2.total, a2.created_by, a2.keterangan,
                        b.kode as transaksi_kode, b.deskripsi as transaksi_nama, c.kode as muzakki_kode, c.nama as muzakki_nama", false);
        $this->db->from('penerimaan_det a');
        $this->db->join('penerimaan a2','a.penerimaan = a2.id');
        $this->db->join('m_kode_transaksi b','a.kode_transaksi = b.id');
        $this->db->join('m_muzakki c','a2.muzakki = c.id');
        $this->db->where('a.is_deleted', 2);
        $this->db->where('a2.is_deleted', 2);
        $this->db->where('a2.status >', 1 , false);
        $this->db->where('a2.cabang', $cabang, false);
        ...
        if (!empty($src_muzakki)) {
            $this->db->where('a2.muzakki', $src_muzakki);
        }
    ...
    function getRekapPenerimaanMuzakki($cabang, $src_date1, $src_date2, $src_muzakki='') {
        
        $tanggal1 = date('Y-m-d', strtotime($src_date1));
        $tanggal2 = date('Y-m-d', strtotime($src_date2));
        
        $this->db->select("c.kode as muzakki_kode, c.nama as muzakki_nama, COALESCE(sum(b.jumlah),0) as total", false);
        $this->db->from('m_muzakki c');
        $this->db->join('penerimaan a',"a.muzakki = c.id AND a.cabang=".$cabang." AND a.tanggal >= '".$tanggal1."' AND a.tanggal <= '".$tanggal2."' AND a.status > 1  AND a.is_deleted=2", 'LEFT');
        $this->db->join('penerimaan_det b','b.penerimaan = a.id AND b.is_deleted=2', 'LEFT');
        $this->db->where('c.is_deleted', 2 , false);
        $this->db->where('c.cabang', $cabang , false);

        if (!empty($src_muzakki)) {
            $this->db->where('c.id', $src_muzakki);
        }
        
        $this->db->group_by('c.kode, c.nama');
        $this->db->order_by('c.kode', 'ASC');
        $query = $this->db->get();
    ...
    function getDataPenyaluranMustahik($cabang, $src_date1, $src_date2, $src_mustahik='') {
        ...
        $this->db->select("a.id as idx, a.kode_transaksi, a.akun_kas, a.mutasi_kas_bank, a.jumlah,
                        a2.no_transaksi, a2.tanggal, a2.total, a2.created_by, a2.keterangan,
                        b.kode as transaksi_kode, b.deskripsi as transaksi_nama, c.kode as mustahik_kode, c.nama as mustahik_nama", false);
        $this->db->from('pengeluaran_det a');
        $this->db->join('pengeluaran a2','a.pengeluaran = a2.id');
        $this->db->join('m_kode_transaksi b','a.kode_transaksi = b.id');
        $this->db->join('m_mustahik c','a2.mustahik = c.id');
        $this->db->where('a.is_deleted', 2);
        $this->db->where('a2.is_deleted', 2);
        $this->db->where('a2.status >', 1 , false);
        $this->db->where('a2.cabang', $cabang, false);
        ...
        if (!empty($src_mustahik)) {
            $this->db->where('a2.mustahik', $src_mustahik);
        }
    ...
    function getRekapPenyaluranMustahik($cabang, $src_date1, $src_date2, $src_mustahik='') {
        
        $tanggal1 = date('Y-m-d', strtotime($src_date1));
        $tanggal2 = date('Y-m-d', strtotime($src_date2));
        
        $this->db->select("c.kode as mustahik_kode, c.nama as mustahik_nama, COALESCE(sum(b.jumlah),0) as total", false);
        $this->db->from('m_mustahik c');
        $this->db->join('pengeluaran a',"a.mustahik = c.id AND a.cabang=".$cabang." AND a.tanggal >= '".$tanggal1."' AND a.tanggal <= '".$tanggal2."' AND a.status > 1 AND a.is_deleted=2", 'LEFT');
        $this->db->join('pengeluaran_det b','b.pengeluaran = a.id AND b.is_deleted=2', 'LEFT');
        $this->db->where('c.is_deleted', 2 , false);
        $this->db->where('c.cabang', $cabang , false);

        if (!empty($src_mustahik)) {
            $this->db->where('c.id', $src_mustahik);
        }
    ...
    function getDataJenisDana($jenis=null) {
        
        $this->db->select("id, nama as jenis_dana", false);
        $this->db->from('m_dana');
    ...
    function getDataPerubahanDana($cabang=null, $bulan=null, $tahun=null, $jenis=null) {
        
        $tanggal1 = $tahun.'-'.$bulan.'-01';
        $tanggal2 = $this->customlib->lastDayOfTheMonth($tanggal1); //$tahun.'-'.$bulan.'-31';
        
        $this->db->select("a.id, a.cabang,a.dana, b.nama jenis_dana,a.saldo_bulan_lalu,d.tipe,d.nama jenis_transaksi,c.jumlah", false);
        $this->db->from('perubahan_dana a');
        $this->db->join('m_dana b','b.id = a.dana');
        $this->db->join('perubahan_dana_det c','c.perubahan_dana = a.id');
        $this->db->join('m_dana_trx d','d.id = c.dana_trx');
        
        $this->db->where('a.periode', $bulan);
        $this->db->where('a.tahun', $tahun, false);
        if (!empty($cabang)) {
            $this->db->where('a.cabang', $cabang, false);
        }
        if (!empty($jenis)) {
            $this->db->where('a.dana', $jenis);
        }
...
//            $q_saldo_awal = $this->db->query("select COALESCE(sum(a.saldo),0) saldo from neraca_saldo a
//                                    where a.periode='".$bulan_before."' and a.tahun=".$tahun_before." and a.akun IN (".$akun_impl.") ".$add_where."");
    ...
    function getReportArusKas($cabang=null, $bulan=null, $tahun=null) {
        ...
        */
        ...
        //get total parent
        $q_reportp = $this->db->query("select a.id, a.kode, a.level, a.parent, a.nama, sum(d.saldo) saldo
                                from m_arus_kas a 
        ...
        // detail
        $q_report = $this->db->query("select a.id, a.level, a.kode, a.parent, a.nama, sum(d.saldo) saldo
                                from m_arus_kas a 
                                left join m_arus_kas_det c on c.arus_kas = a.id 
    ...
    function getMuzakkiById($src_muzakki) {

        $this->db->where('id', $src_muzakki);
        $query = $this->db->select('*')->get('m_muzakki');
        $data = $query->row_array();
        return $data['nama'];
    }
    
    function getMustahikById($src_mustahik) {

        $this->db->where('id', $src_mustahik);
        $query = $this->db->select('*')->get('m_mustahik');
        $data = $query->row_array();
        return $data['nama'];
    }
    
    function getMuzakkiByCabang($cabang='') {
    ...
    function getMustahikByCabang($cabang='') {
