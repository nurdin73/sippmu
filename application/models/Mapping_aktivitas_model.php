 ...
 * 
 */
class Mapping_aktivitas_model extends CI_model {

    function getAll() {
        
        $this->datatables->select("a.id as id, '' as nomor, a.nama, a.kode, a.parent, a.level, a.urutan, a.is_sub_total, '' as view", false);
        $this->datatables->from('report_aktivitas a');
...
//                                                        <i class="fa fa-trash"></i></a>', 'id');
    ...
    function getAllDetail() {
        $aktivitas_id = $this->input->post('aktivitas_id') ? $this->input->post('aktivitas_id') : '';
       
        if(!empty($aktivitas_id)){
        
            $this->datatables->select("a.id as id, '' as nomor, b.kode as kode_rek, b.nama as nama_rek", false);
            $this->datatables->from('report_aktivitas_det a');
            $this->datatables->join('m_akun b','a.id_rek = b.id');
            $this->datatables->where('a.report_aktivitas', $aktivitas_id);
    ...
    function getData($id = null) {
        ...
        if (isset($id)) {
            ...
            $query = $this->db->get('report_aktivitas a');

            return $query->row_array();
        } else {
            $this->db->order_by('urutan', 'ASC');
            $query = $this->db->get("report_aktivitas");
    ...
    function getDataDetail($id = null) {
        ...
        if (isset($id)) {
            ...
            $this->db->where("a.id", $id);
            $query = $this->db->get('report_aktivitas_det a');
            return $query->row_array();
        } else {

            $query = $this->db->get("report_aktivitas_det");
            return $query->result_array();
        }
    }
    ...
    public function addData($data) {
        ...
        $this->db->insert('report_aktivitas', $data);
    ...
    public function editData($data, $id) {
        ...
        if (isset($id)) {
            $this->db->where('id', $id);
            $this->db->update('report_aktivitas', $data);
        } 
        return $id;
    }
    ...
    public function addDataDetail($data) {
        ...
        //===insert ke tabel report_aktivitas_det
        $this->db->insert('report_aktivitas_det', $data);
        $aktivitas_det_id = $this->db->insert_id();
        
        return $aktivitas_det_id;
    }

    public function editDataDetail($data, $aktivitas_det_id) {
        
        if (isset($aktivitas_det_id)) {
            //=== update ke tabel report_aktivitas_det
            $this->db->where('id', $aktivitas_det_id);
            $this->db->update('report_aktivitas_det', $data);
        }
    ...
    function deleteData($id) {
        if (isset($id)) {
            ...
            $this->db->where('id', $id);
            $dodelete = $this->db->delete('report_aktivitas');

            if($dodelete){
                $this->db->where('report_aktivitas', $id);
                $this->db->delete('report_aktivitas_det');
            }
        } 
    ...
    function deleteDataDetail($id, $aktivitas_id) {
        //validasi : data bisa dihapus dengan kondisi status=1 
        if (isset($id)) {
            
            $this->db->where('id', $id);
            $this->db->where('report_aktivitas', $aktivitas_id);
            $dodelete = $this->db->delete('report_aktivitas_det');
    ...
    function getParent() {
        ...
        $query = $this->db->select('id,kode, nama')->get('report_aktivitas');
        return $query->result_array();
    }
    ...
    function getKodeParent($id = null) {
        ...
        if (isset($id)) {
            ...
            $this->db->order_by('urutan', 'ASC');
            $query = $this->db->where("id", $id)->get('report_aktivitas');
            ...
            $query2 = $this->db->get('report_aktivitas');
