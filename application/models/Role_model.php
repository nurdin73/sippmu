...
///======================
    ...
    public function getPermissions($group_id, $role_id) {
        ...
        $sql = "SELECT permission_category.*,CASE WHEN roles_permissions.id = 0 THEN '0' ELSE roles_permissions.id END as roles_permissions_id,roles_permissions.can_view,roles_permissions.can_add ,roles_permissions.can_edit ,roles_permissions.can_delete 
                FROM permission_category 
                LEFT JOIN roles_permissions on permission_category.id = roles_permissions.perm_cat_id and roles_permissions.role_id= $role_id 
                WHERE permission_category.is_active = 1 AND permission_category.perm_group_id = $group_id ORDER BY permission_category.id";
    ...
    public function count_roles($id) {
        ...
        $query = $this->db->select("*")->join("users", "users.id = user_roles.user_id")->where("user_roles.role_id", $id)->where("users.is_active", 1)->get("user_roles");
