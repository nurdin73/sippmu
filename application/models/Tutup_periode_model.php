...
class Tutup_periode_model extends CI_model {
    ...
    public function closingTutupPeriode($cabang, $periode_bln, $periode_thn) {
        ...
        if (!empty($cabang)) {
			...
			//update dony 20210711
            $periode_date1 = $periode_thn.'-'.$periode_bln.'-01'.' 00:00:01';
            $periode_date2a = $this->customlib->lastDayOfTheMonth($periode_date1);
            $periode_date2 = $periode_date2a.' 23:59:59';
            ...
            */   
            ...
            ### update akun akun dana di tabel neraca saldo, berdasarkan data sbb :
			    //update dony
            /*
				$q_dana_det_cek = $this->db->query("select a.cabang,b.akun_dana,sum(a.jumlah) jumlah 
                ...
                if($data_cek_dana){
                    ...
                    foreach($data_cek_dana as $cek){
                        ...
                        $data_saldo = array();
                        if($cek['jumlah'] < 0){
                            $data_saldo['mutasi_debet'] = $cek['jumlah'];
                            $data_saldo['mutasi_kredit'] = 0;//$mutasi_kreditx;
                        }
                        if($cek['jumlah'] > 0){
                            $data_saldo['mutasi_kredit'] = $cek['jumlah'];
            ...
            */  
            ...
            ###- insert ke neraca saldo dengan periode bulan berikutnya
                ...
                if($data_neraca){
                    ...
                    if (!empty($to_be_insert)) {
                        ...
                        #PERUBAHAN MODAL
                        //klo akhir periode
                        //akun2 pendapatan & biaya diset 0
                        //akun laba/rugi tahun berjalan diset selisih pendapatan - biaya
                        //...... next
            ...
            $result = array('success' => 'false','message'=>'Cabang tidak ditemukan!');
        }
